...
export const checkAllowanceBuyGem = async (setBoolApprove) => {
	const web3 = getWeb3();
	const address = sessionData.myAddress;
	if (web3 !== null && address !== null) {
		const ninneko = new web3.eth.Contract(TokenABI, NinnekoAddress);
		await ninneko.methods
			.allowance(address, SapphireShopAddress)
			.call({ from: address })
			.then((data) => {
				setBoolApprove(data > 0);
			});
	} else setBoolApprove(false);
};
...
export const approveBuyGemNft = (snackbar, onComplete, onError) => {
	const web3 = getWeb3();
	const ninneko = new web3.eth.Contract(TokenABI, NinnekoAddress);
	handleResult(ninneko.methods.approve(SapphireShopAddress, MAX_APPROVE), snackbar, onComplete, onError);
};

export const buyGemNftBox = (amount, snackbar, onComplete, onError, priceValue) => {
	const web3 = getWeb3();
